...
תמיד שימוש ב micro-agents ולא ב monolith agents

יש להגדיר API Contract מפורש (כגון Interface / ABC / Schema / Structured Output) רק בנקודות גבול מהותיות בין רכיבים עצמאיים, כלומר בכל מקום שבו רכיב אחד נסמך על רכיב אחר שעשוי להתחלף, להתפתח, או להיות ממומש על-ידי אדם/צוות אחר (Planner↔Executor, Agent↔Tool, Agent↔Agent, LLM↔קוד). במקרים אלו החוזה חייב להגדיר קלט, פלט ומשמעות סמנטית באופן חד ומאכף. לעומת זאת, אין להגדיר חוזים פורמליים לקוד פנימי, helpers או לוגיקה מקומית שאינם מהווים נקודת אינטגרציה, כדי להימנע מ-over-engineering ופגיעה בפרודוקטיביות. העיקרון המנחה: אם ניתן או סביר להחליף מימוש בלי לשבור את המערכת – נדרש Contract; אם לא – חוזה מרומז וקריאות קוד מספיקים.
...
לבדוק אם יש לי בדיקות של TIME בקבצי RULE ולבדוק אם כדאי להמיר את זה באמצעות הוספת חותמת זמן ב LOGGER. 

Profiler הוא כלי מדידה שמריצים על קוד כדי **לאתר צווארי בקבוק אמיתיים בביצועים במקום לנחש**; באמצעות כלים כמו `cProfile` ניתן לראות בדיוק אילו פונקציות צורכות את מירב זמן הריצה (בעיקר לפי העמודה `totTime`), כמה פעמים הן נקראות (`nCalls`) ומה הזמן המצטבר שלהן (`cumTime`), וכך לזהות במהירות קטעי קוד בעייתיים. הניתוח מראה לרוב שקוד שעובד היטב על דאטה קטן נכשל בסקייל בגלל **סיבוכיות אלגוריתמית שגויה** (למשל לולאה בתוך לולאה → ‎O(n²)), ולא בגלל תשתיות או דאטה־בייס. המסקנה ההנדסית היא ש**כל אופטימיזציה חייבת להתחיל במדידה**: מריצים Profiler, מזהים פונקציה אחת או שתיים שמהוות bottleneck, מבינים את ה־Big-O שלהן, ורק אז מבצעים ריפקטור ממוקד שמייצר שיפור משמעותי בפרודקשן.
ליצור עבורו COMMAND
