...
Governance pass before commit: map the current change set to **applicable Rules** under `.cursor/rules/` (agents, API, data, configuration, infrastructure, security, tests). This command covers **cross-cutting** expectations. It **does not** duplicate line-by-line code review—that is owned by `/review/code-review-checklist`.
