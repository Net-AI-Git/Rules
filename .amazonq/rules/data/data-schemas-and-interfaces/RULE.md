See rule: contract-scope-and-boundaries (in .amazonq/rules) for when to define explicit schemas (boundary points such as Agent↔Tool, LLM↔code).
...
* **dataclass:** Use for internal structures—state objects, config, results, descriptive objects (e.g., Plan, Action, Score). Less boilerplate, no validation overhead. See rule: contract-scope-and-boundaries (in .amazonq/rules) for boundary vs internal distinction.
