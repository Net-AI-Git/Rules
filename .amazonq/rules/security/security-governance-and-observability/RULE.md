...
* **Rate Limits:** Enforce budgets and rate limits to prevent runaway loops. See rule: api-interface-and-streaming (in .amazonq/rules) for implementation details.

* **Auditing:** Enable full auditing and tracing to attribute every action to a specific agent and context. Use **structlog** for all logging and audit trail construction. See rule: monitoring-and-observability (in .amazonq/rules) for comprehensive auditing and tracing strategies; See rule: audit-protocol (in .amazonq/rules) for audit log structure.
...
* See rule: llm-evaluation-and-metrics (in .amazonq/rules) for comprehensive evaluation strategies.
...
* **Input Validation:**
    * **Pydantic Models:** Use Pydantic models for request validation (See: (See rule: data-schemas-and-interfaces in .amazonq/rules)).
...
* **FastAPI Implementation:**
    ...
    * **App Structure:** For app structure and middleware order, See rule: api-interface-and-streaming (in .amazonq/rules).
