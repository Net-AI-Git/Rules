...
* **Built with structlog:** Audit log entries are constructed using **structlog** for consistent structured logging. Use structlog's bound loggers and processors to build entries with required fields.
...
* **Immutable Logs:**
  ...
  * Implement cryptographic hashing (SHA-256) for log integrity verification.

For examples see the file `examples_audit_logging.py` in this folder. When using this rule, add the relevant example file(s) to the chat context. for audit log structure and event logging implementation pattern using structlog.

### Audit Log Storage

* **Centralized Storage:** Store audit logs in **Splunk** as the default SIEM and centralized audit store. Send audit events via **Splunk HEC** (HTTP Event Collector) to a dedicated index (e.g., `audit`). Configure retention in Splunk per regulatory requirements (minimum 7 years for financial data, 3 years for general business).
* **Retention Policy:** Define retention and backup in Splunk (immutable storage, backup policies as per Splunk and compliance).
* **Access Control:** Restrict access to audit logs in Splunk (read-only for auditors, no deletion permissions).
...
For examples see the file `examples_compliance.py` in this folder. When using this rule, add the relevant example file(s) to the chat context. for compliance validation pattern implementation.

## 3. Logging & Tracing Standards

### Structured Logging

* **Log Format:** Use **structlog** with JSON rendering for all audit-related logs.
  * **Consistency:** Maintain consistent log structure across all services using structlog processors.
  * **Parsing:** Enable easy parsing and querying of logs in Splunk via SPL.
...
* **Splunk as Tracing Backend:**
  * **Span Events:** Emit span/trace events to **Splunk** (via HEC) for all significant operations (tool calls, API requests, database queries).
  * **Span Attributes:** Include audit-related attributes (user ID, tenant ID, action type, correlation_id, operation.name, duration_ms).
  * **Trace Context:** Propagate correlation_id across all service boundaries; use SPL to correlate events by correlation_id.

* **Trace Retention:**
  * **Duration:** Retain traces in Splunk for at least 30 days (longer for compliance); configure retention in Splunk.
  * **Sampling:** Use intelligent sampling (100% for audit events, lower for general operations) when emitting to Splunk.

### Log Aggregation

* **Centralized Logging:**
  * **SIEM Integration:** Send audit logs to **Splunk** (via HEC). Use Splunk as the single SIEM for centralized security monitoring.
  * **Real-Time Monitoring:** Use Splunk real-time search and dashboards to monitor audit events for anomalies.
  * **Alerting:** Use **Splunk Alerts** (SPL-based) for suspicious patterns (unusual access, failed authorization attempts).

* **Log Analysis:**
  * **Query Interface:** Use **SPL** (Search Processing Language) in Splunk for auditors to search and analyze audit logs.
  * **Visualization:** Create Splunk dashboards for audit log analysis and compliance reporting.
  * **Anomaly Detection:** Use SPL and Splunk capabilities for anomaly detection in audit logs.
...
For examples see the file `examples_forensics.py` in this folder. When using this rule, add the relevant example file(s) to the chat context. for timeline reconstruction and forensic analysis pattern.
...
* **Security Systems:**
  * **SIEM Integration:** Use **Splunk** as the SIEM for centralized security monitoring; all audit logs are sent to Splunk via HEC.
  * **IAM Integration:** Integrate with Identity and Access Management systems for user context.
  * **Threat Intelligence:** Feed audit data into threat intelligence systems as needed.
...
* **Monitoring Systems:**
  * See rule: monitoring-and-observability (in .amazonq/rules) for comprehensive monitoring strategies (Splunk and SPL).
  * See rule: security-governance-and-observability (in .amazonq/rules) for security governance requirements.
