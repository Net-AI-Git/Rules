...
- **Apply Manually** rules must be tagged with rule: rule-name (in .amazonq/rules) if needed
...
4. **Apply Manually Rules**: Must be tagged with rule: rule-name (in .amazonq/rules) if needed
...
## Data Sources
- Dependency files (`pyproject.toml`, `poetry.lock`); if present, `requirements.txt` for legacy projects
- Source code files
...
When a user tags rule: commands-management (in .amazonq/rules) or asks you to create/update a command:
