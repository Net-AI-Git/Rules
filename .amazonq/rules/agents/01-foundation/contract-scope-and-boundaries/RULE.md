...
See rule: agent-component-interfaces (in .amazonq/rules) for Planner, Memory, Executor interface definitions.

See rule: data-schemas-and-interfaces (in .amazonq/rules) for Pydantic schemas and structured outputs for tools and LLM.
...
* **Component Interfaces:** rule: agent-component-interfaces (in .amazonq/rules) defines contracts for Planner, Memory, Executor—all boundary points per this rule.
* **Data Schemas:** rule: data-schemas-and-interfaces (in .amazonq/rules) defines schemas for tools and LLM outputs—boundary points (Agent↔Tool, LLM↔Code).
* **Multi-Agent:** rule: multi-agent-systems (in .amazonq/rules) defines SECTIONS and Agent↔Agent boundaries; contracts apply at those boundaries.
