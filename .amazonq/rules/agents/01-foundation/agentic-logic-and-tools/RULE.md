...
## Agent Internals
* **Structured Output in LangGraph Nodes:** When implementing agent logic **inside a LangGraph node**, use **Pydantic BaseModel** for structured outputs (define schemas with `Field(description="...")` for LLM binding). Use `with_structured_output()` or `.bind_tools()` for LLM integration. See: rule: data-schemas-and-interfaces (in .amazonq/rules) for schema standards.
