# תיעוד כללי — חוקי Amazon Q Developer (בעברית)

זה המסמך בעברית לכל **חוקי הפרויקט ל-Amazon Q Developer** (`.amazonq/rules`). כאן תמצא: מה כל חוק אומר, **מתי להשתמש** בו, ואילו **ספריות וטכנולוגיות** הוא מגדיר. המטרה: שכל פעם שתרצה להבין איזה חוק רלוונטי עכשיו — זה יהיה ברור וקל לגשת.

**למחליטים (ראשי צוות, מפתחים חיצוניים):** תחת כל חוק בפירוט יש גם סעיף **"למחליט (האם לאמץ?)"** — הסבר קצר בשפה פשוטה: מה החוק נותן לקוד ולצוות, ומתי כדאי לאמץ אותו.

**פרטים טכניים** (מבנה תיקיות, פורמט Markdown בלי frontmatter, איך לכתוב חוקים) — ראה [README.md](README.md) ו-[rules-management/amazon-q-developer-rules-guide.md](rules-management/amazon-q-developer-rules-guide.md) (באנגלית).
...
| **rules-management** | יצירה/עדכון של Rules (פורמט, מבנה) | לפי קבצים (RULE.md) | — |
...
| **commands-management** | יצירה/עדכון של Commands | לפי קבצים (.amazonq/commands/*.md) | — |
