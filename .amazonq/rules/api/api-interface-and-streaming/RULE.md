...
## 2. Framework Choice (Default Stack)

* **Framework:** Use **FastAPI** as the standard server framework (mandate for all API code in this project).

* **Supporting Libraries:**
    * **Pydantic:** Use for request/response models and validation (built into FastAPI). Define schemas for all API boundaries.
    * **OpenAPI/Swagger:** Rely on FastAPI's automatic OpenAPI schema generation; expose at `/openapi.json` and `/docs`.

* **ASGI Server:** Use **Uvicorn** as the ASGI server to run the FastAPI app (development and production). Do not reference Starlette directly—it is the underlying layer of FastAPI. For Uvicorn installation (`uvicorn[standard]`), development (`--reload`), and production (`--workers`) defaults, **see:** rule: uvicorn-asgi-server (in .amazonq/rules).

* **Streaming:** Use FastAPI's `StreamingResponse` for SSE or WebSocket support. Endpoints must be implemented as `async def` that `yield` chunks (generators) or return a `StreamingResponse`.

* For examples see the file `examples_fastapi_streaming.py` in this folder. When using this rule, add the relevant example file(s) to the chat context. for a minimal FastAPI app with SSE and rate limiting.
...
* See rule: security-governance-and-observability (in .amazonq/rules) for governance-level rate limiting requirements. rule: api-documentation-standards (in .amazonq/rules) for documenting rate limits in API documentation.
