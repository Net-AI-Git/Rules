...
For examples see the file `examples_prompt_versioning.py` in this folder. When using this rule, add the relevant example file(s) to the chat context. for version control patterns and A/B testing implementation.
...
For examples see the file `examples_prompt_testing.py` in this folder. When using this rule, add the relevant example file(s) to the chat context. for unit testing patterns and test framework implementation.

## 5. Prompt Registry and Splunk Observability

* **Mandate:** Use a **Prompt Registry** as the single source of truth for prompts (YAML files, app config, or in-app registry). All prompt usage, version changes, and performance metrics MUST be sent to **Splunk** (via HEC) for audit and analysis.

* **Prompt Registry:**
  * **Centralized Storage:** Store prompts in YAML, config, or a registry module; version and history are tracked and emitted to Splunk as events.
  * **Version Management:** Track versions and changes; log version selection and updates to Splunk for audit.
  * **Retrieval:** Retrieve prompts from the registry at runtime; log retrieval events (prompt id, version, correlation_id) to Splunk.

* **Splunk Integration:**
  * **Audit and History:** Send prompt load/retrieval events and version changes to Splunk via HEC (index e.g. `audit` or `prompts`).
  * **Performance Metrics:** Measure and send latency, cost, and quality metrics per prompt version to Splunk; build dashboards and run analysis using **SPL**.

**See:** @examples_prompt_registry_splunk for Prompt Registry with Splunk logging pattern.
...
For examples see the file `examples_prompt_versioning.py` in this folder. When using this rule, add the relevant example file(s) to the chat context. for YAML management patterns.
...
For examples see the file `examples_prompt_versioning.py` in this folder. When using this rule, add the relevant example file(s) to the chat context. for runtime prompt version selection implementation.
...
For examples see the file `examples_prompt_versioning.py` in this folder. When using this rule, add the relevant example file(s) to the chat context. for in-loop metrics collection implementation.
...
For examples see the file `examples_prompt_versioning.py` in this folder. When using this rule, add the relevant example file(s) to the chat context. for runtime A/B testing manager implementation.
