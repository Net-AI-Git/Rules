...
* [ ] **API Docs:** Is OpenAPI/Swagger documentation complete? (FastAPI provides `/docs` and `/openapi.json` See rule: api-documentation-standards (in .amazonq/rules).)
...
* [ ] **Containerization:** Are Docker images built per project defaults (See rule: deployment-and-infrastructure (in .amazonq/rules)): multi-stage when applicable, non-root user, dependency layers before code for cache, no secrets in image, logs to stdout/stderr, and image scan (e.g. Trivy/Snyk) in CI?
