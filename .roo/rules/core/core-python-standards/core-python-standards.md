...
4.  **Logging Setup:** Configure structured JSON logging sent to **Splunk via HEC** as the centralized observability platform (see @monitoring-and-observability).
...
* **CPU Bound (Data Processing/Embeddings):**
    ...
    * **See:** `@examples_process_pool.py` for complete ProcessPoolExecutor implementation patterns including synthetic data generation and parallel agent initialization for production systems.
    * Offload heavy computations to separate processes so the Agent loop doesn't freeze.

### 3. Logging System
* **No `print()`:** All logging must produce structured JSON events sent to **Splunk via HEC** (HTTP Event Collector).
* **HEC Direct:** Use the project's Splunk HEC client (HTTP POST with JSON). Every event MUST include: `timestamp` (ISO 8601, UTC), `correlation_id` (UUID), `operation_name`.
* **Timed operations:** Also include `start_time`, `end_time`, `duration_ms` (use `time.perf_counter()`).

* **See:** @monitoring-and-observability for mandatory fields, metrics, naming conventions, and alerting. Use `@splunk-instrumentation` skill to add instrumentation to existing code.
...
## Final Check
Before submitting code, verify: all functions are under 20 lines, all I/O is async, all types are hinted, logging uses Splunk HEC (no `print()`), and error handling follows `error-handling-and-resilience` (retry with backoff, circuit breaker, error classification).
