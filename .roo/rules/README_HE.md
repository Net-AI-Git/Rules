...
| **commands-management** | יצירה/עדכון של Commands | לפי קבצים (.roo/commands/*.md) | — |
...
**Skills (לא חוקים):** LLM-as-a-Judge — `@evaluate-with-llm-judge` (קובץ: `.roo/skills/evaluate-with-llm-judge/SKILL.md`).
...
- **מה זה:** פרוטוקול LLM-as-a-Judge מלא (Supreme AI Adjudicator): קלט Session Context, רובריקה משוקללת, `<reasoning>`, פלט JSON מובנה. אין חוק נפרד — הכל בקובץ ה-Skill תחת `.roo/skills/evaluate-with-llm-judge/`.
...
- **מתי נשתמש:** Apply to Specific Files — `**/.roo/rules/**/RULE.mdc`.
...
#### commands-management
- **מה החוק אומר:** פורמט ומבנה ל־Cursor Commands: קבצי .md ב־.roo/commands, כותרת H1, תיאור, שלבים, Rules Applied.
- **מתי נשתמש:** Apply to Specific Files — `**/.roo/commands/**/*.md`.
- **ספריות וטכנולוגיות:** אין.
...
- **[Commands](../commands/slash-commands-documentation.md)** — תיעוד Slash Commands (Roo).
