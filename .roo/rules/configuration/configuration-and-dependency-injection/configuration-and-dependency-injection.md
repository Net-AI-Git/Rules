...
* **No Leaks:** Never log config objects without masking sensitive fields (using Pydantic's `SecretStr`).

* **Kubernetes:** In Kubernetes deployments, application **MUST** read configuration from environment variables; Kubernetes injects these from ConfigMaps and Secrets. Do not assume config files on disk for runtime secrets or environment-specific values. **See:** @deployment-and-infrastructure.
