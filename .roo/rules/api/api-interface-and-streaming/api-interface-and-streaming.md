...
* **ASGI Server:** Use **Uvicorn** as the ASGI server. Do not use another ASGI server or `python main.py`. Ensure `pyproject.toml` declares `uvicorn[standard]`, not plain `uvicorn`.
    * **uvloop** — Replaces Python's default asyncio event loop (x2-4 faster, based on libuv).
    * **httptools** — Replaces h11 HTTP parser (x5-10 faster, C-based).
    * **watchfiles** — Rust-based file watcher for reliable `--reload` in development.
...
* **See:** `@examples_fastapi_streaming.py` for a minimal FastAPI app with SSE and rate limiting.
...
* **Policy:** `@security-governance-and-observability` requires API/agent throttling to exist; **implementation** is this section (and `@cost-and-budget-management` for in-workflow token budgets). `@api-documentation-standards` for documenting rate limits in API documentation.
